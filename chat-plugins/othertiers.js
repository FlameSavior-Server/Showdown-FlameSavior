...
exports.commands = {
	...
	othertiers: function (target, room, user, connection) {
		...
		var ot = toId(target);
		var reply = "";
		if (ot === 'poll') return user.chat('/poll Next type of Tour?, Inverse battle, Little Cup, Ubers, Smogon Doubles, Balanced Hackmons, RU, NU, UU, PU, Hackmons, Catch and Evolve, random doubles battle, Skybattles,Random Triples Battle,Random Monotype,Challenge Cup, Doubles Challenge Cup,Random LC,Metronome,Triples Challenge Cup, Seasonal,Anything Goes,Gen 1 Randbat,Hackmons Challenge Cup,battle spot,random battle perseverance',room,connection);
		...
		switch (ot) {
			...
			case 'perseverance': reply +=	"Play until one mon dies then forfeit. More detailed rules are <a href=\"http://pastebin.com/JGSD9T4n\">here</a>."; break;
		}
