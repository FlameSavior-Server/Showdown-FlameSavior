...
var parse = exports.parse = function(message, room, user, connection, levelsDeep) {
                ...
                if (broadcast) {
                    ...
                    if (!color.has(user.userid)) {
                        ...
                        if (user.getIdentity(room.id).substring(0, 1) !== ' ') {
                            ...
                            else room.add('|html|<button class="userbutton" name="parseCommand" value="/user ' + user.getIdentity() + '"><b><font color = ' + usercolor + '>' + user.name + ': </font></b></button> ' + (formatMessage(suppressMessage) || formatMessage(message)));
                        }
                    }
    ...
    if (room && color.has(user.userid)) {
        ...
        if (user.getIdentity().substring(0, 1) !== ' ') {
            if (room.battle) {
                add = room.add('|html|<button name="parseCommand" class="userbuttonbattle" value="/user ' + user.getIdentity() + '"><font color = "gray">' + user.getIdentity().substr(0, 1) + '</font><b><font color = ' + color.get(user.userid) + '>' + user.name + ': </font></b></button> ' + message);
            } else {
                add = room.add('|html|<button name="parseCommand" class="userbutton" value="/user ' + user.getIdentity() + '"><font color = "gray">' + user.getIdentity().substr(0, 1) + '</font><b><font color = ' + color.get(user.userid) + '>' + user.name + ': </font></b></button> ' + message);
            }
            return add;
        } else {
            if (room.battle) {
                add = room.add('|html|<button name="parseCommand" class="userbuttonbattle" value="/user ' + user.getIdentity() + '"><b><font color = ' + color.get(user.userid) + '>' + user.name + ': </font></b></button> ' + message);
            } else {
                add = room.add('|html|<button name="parseCommand" class="userbutton" value="/user ' + user.getIdentity() + '"><b><font color = ' + color.get(user.userid) + '>' + user.name + ': </font></b></button> ' + message);
            }
            return add;
        }
