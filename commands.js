	...
	makechatroom: function (target, room, user, connection, cmd) {
		...
		if (Rooms.global.addChatRoom(target)) {
			if (cmd === 'makeprivatechatroom') {
				...
				let defaultIntro = '<h2 style="margin-top:0">' + target + ' <a href="/' + toId(target) + '"><code>&lt;&lt;' + toId(target) + '>></code></a></h2>(DEFAULT ROOMINTRO)<br />- To set a customized roomintro, use /roomintro <code>HTML</code>.<br />- Don\'t know HTML very well? Click <a href="http://www.smogon.com/forums/threads/html-jazz-b.3510885/">here</a> for a brief HTML guide.<br />- Be sure to set a short room description (80 characters or under) with /roomdesc <code>DESC</code><br />- Reminder that this room must maintain 40 messages / 48 hours to be considered "active" to not be automatically deleted.<br />- For any <i>general</i> questions on room management, please do not hesitate to contact a global staff member.<br />- <button name="send" value="/roomhelp">Room management commands</button>';
