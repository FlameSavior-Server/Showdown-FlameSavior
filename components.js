...
var components = exports.components = {
	...
	away: function(target, room, user) {
		...
		if (user.isAway) {
			...
			if (user.substr(-statusLen) === status) return this.sendReply('Your away status is already set to "' + target + '".');
		}
	...
	back: function (target, room, user) {
		...
		user.updateIdentity();
		user.isAway = false;
		if (user.can('lock', null, room)) this.add('|raw|-- <font color="#088CC7"><strong>' + Tools.escapeHTML(newName) + '</strong></font> is no longer ' + status.toLowerCase() + '.');
	},
